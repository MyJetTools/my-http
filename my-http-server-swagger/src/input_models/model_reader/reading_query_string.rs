        ...
        PropertyType::OptionOf(sub_ty) => {
            ...
            let result = quote::quote! {
                let #let_input_param = if let Some(value) = #data_src.get_optional(#input_field_name) {
                    let value = my_http::core::InputParamValue::from(value);
                    Some(value.try_into()?)
                } else {
                    #default_value
                };
    ...
    if let Some(default_value) = input_field.get_default_value()? {
        match default_value {
            ...
            crate::input_models::DefaultValue::Value(default) => {
                ...
                let result = quote::quote! {
                    let #let_input_param = if let Some(value) = #data_src.get_optional(#input_field_name){
                        my_http::core::InputParamValue::from(value).try_into()?
                    }else{
                        #else_data
                    };
                };
        ...
        return Ok(
            quote::quote!(let #struct_field_name: #ty = my_http::core::InputParamValue::from(#data_src.get_required(#input_field_name)?).try_into()?;),
        );
    }
}
